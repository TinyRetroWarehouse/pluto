...
pub fn ComptimeBitmap(comptime BitmapType: type) type {
    return struct {
        ...
        ///     IN from: ?usize - The entry number to start allocate from or null if it can start anywhere
        ///
        /// Return: ?IndexType
        ...
        ///
        pub fn setContiguous(self: *Self, num: usize, from: ?usize) ?IndexType {
            ...
            var bit: IndexType = @intCast(IndexType, from orelse 0);
            while (true) {
                const entry = bit;
